...
import { CompanyDto } from 'src/app/core';
  ...
  export class MapComponent {
      ...
      this.map?.on('load', () => {
        ...
        if (!this.map?.getSource('water-bodies')) {
          this.map?.addSource('water-bodies', {
            type: 'geojson',
            data: 'https://hector-gallego.github.io/api-json/data_water_bolivar_4.geojson',
          });
    ...
    loadPopupAndMarker(company: CompanyDto, distRiver: number): void {

      const url = `${environment.localUrl}/company/detail`;
      const economyStr = this.companyService.getEconomyActivitiesDto(company);
